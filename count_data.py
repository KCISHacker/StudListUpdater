...
def count(data: list, grade: dict) -> dict:
    ...
    for datum in data:
        ...
        if first_two_digits not in grade:
            ...
            grade.update({first_two_digits: grade_input})
            continue
        datum.update({"grade": grade.get(first_two_digits)})

        processed_data.append(datum)

    result.update(
        {"last_update": current_date, "count": len(data), "data": processed_data}
    )
    return result
